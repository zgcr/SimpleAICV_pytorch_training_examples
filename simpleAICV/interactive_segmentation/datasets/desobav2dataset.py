...
class DESOBAV2Dataset(Dataset):
    ...
    def __init__(self,
        ...
        self.images_root_dir = os.path.join(root_dir, 'shadow_images')
        self.object_mask_root_dir = os.path.join(root_dir, 'object_masks')
        self.shadow_mask_root_dir = os.path.join(root_dir, 'shadow_masks')

        self.all_image_name_list = set()
        self.all_object_image_name_to_image_mask_path_dict = collections.OrderedDict(
        )
        for per_image_name in os.listdir(self.images_root_dir):
            if '.jpg' in per_image_name:
                per_image_path = os.path.join(self.images_root_dir,
                                              per_image_name)
                per_object_mask_path = os.path.join(self.object_mask_root_dir,
                                                    per_image_name)
                per_shadow_mask_path = os.path.join(self.shadow_mask_root_dir,
                                                    per_image_name)

                if os.path.exists(per_image_path) and os.path.exists(
                        per_object_mask_path) and os.path.exists(
                            per_shadow_mask_path):
                    self.all_image_name_list.add(per_image_name)
                    self.all_object_image_name_to_image_mask_path_dict[
                        per_image_name] = [
                            per_image_path,
                            per_object_mask_path,
                            per_shadow_mask_path,
                        ]
    ...
    def load_image(self, idx):
        image_path = self.all_object_image_name_to_image_mask_path_dict[
    ...
    def load_box_type_mask_type_mask(self, idx):
        object_mask_path = self.all_object_image_name_to_image_mask_path_dict[
            self.all_image_name_list[idx]][1]
        shadow_mask_path = self.all_object_image_name_to_image_mask_path_dict[
            self.all_image_name_list[idx]][2]

        object_mask = np.array(Image.open(object_mask_path).convert('L'),
                               dtype=np.uint8)
        object_mask[object_mask >= 255] = 255
        object_mask[object_mask <= 0] = 0

        shadow_mask = np.array(Image.open(shadow_mask_path).convert('L'),
                               dtype=np.uint8)
        shadow_mask[shadow_mask >= 255] = 255
        shadow_mask[shadow_mask <= 0] = 0

        assert object_mask.shape[0] == shadow_mask.shape[
            0] and object_mask.shape[1] == shadow_mask.shape[1]

        full_mask = np.zeros((object_mask.shape[0], object_mask.shape[1]),
                             dtype=np.float32)
        full_mask = full_mask + object_mask
        flag_mask = (full_mask == 0.)
        full_mask = full_mask + shadow_mask * flag_mask
...
if __name__ == '__main__':
    ...
    from tools.path import desoba_v2_dataset_path
    ...
    desoba_v2_dataset = DESOBAV2Dataset(
        desoba_v2_dataset_path,
        load_mask_type='full_mask',
        load_box_type='object_mask',
        positive_points_num=9,
    ...
    count = 0
    for per_sample in tqdm(desoba_v2_dataset):
    ...
    train_loader = DataLoader(desoba_v2_dataset,
                              batch_size=4,
                              shuffle=True,
                              num_workers=2,
                              collate_fn=collater)
