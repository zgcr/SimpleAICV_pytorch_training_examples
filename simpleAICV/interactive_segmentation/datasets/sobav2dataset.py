...
class SOBAV2Dataset(Dataset):
    ...
    def __init__(self,
                 ...
                 area_filter_ratio=0.000025,
                 box_noise_pixel=50,
                 mask_noise_pixel=50,
                 read_filtered_json=False,
                 save_filtered_json=False,
                 transform=None):
        for per_set_type in set_types:
            assert per_set_type in ['train', 'val',
                                    'challenge'], 'Wrong set name!'
        assert load_mask_type in ['full_mask', 'object_mask', 'shadow_mask']

        set_type_to_name_dict = {
            'train': 'train_relation_v2',
            'val': 'val_v2',
            'challenge': 'challenge_v2',
        }

        self.images_root_dir = os.path.join(root_dir, 'SOBA')
        self.annotations_root_dir = os.path.join(root_dir, 'annotations')

        self.all_root_dir_image_name_to_image_mask_path_dict = collections.OrderedDict(
        )
        for root, folders, files in os.walk(self.images_root_dir):
            for per_image_name in files:
                if '.jpg' in per_image_name:
                    per_image_path = os.path.join(root, per_image_name)
                    per_image_full_mask_path = per_image_path.replace(
                        '.jpg', '-1.png')
                    per_image_object_mask_path = per_image_path.replace(
                        '.jpg', '-2.png')
                    per_image_shadow_mask_path = per_image_path.replace(
                        '.jpg', '-3.png')
                    if os.path.exists(per_image_path) and os.path.exists(
                            per_image_full_mask_path) and os.path.exists(
                                per_image_object_mask_path) and os.path.exists(
                                    per_image_shadow_mask_path):
                        self.all_root_dir_image_name_to_image_mask_path_dict[
                            per_image_name] = [
                                per_image_path,
                                per_image_full_mask_path,
                                per_image_object_mask_path,
                                per_image_shadow_mask_path,
                            ]

        self.all_used_image_name_to_image_mask_path_dict = collections.OrderedDict(
        )
        for per_set_type in set_types:
            per_set_annot_dir = os.path.join(
                self.annotations_root_dir,
                f'SOBA_{set_type_to_name_dict[per_set_type]}.json')
            with open(per_set_annot_dir, encoding='utf-8') as f:
                per_set_json_data = json.load(f)
                per_set_images_info = per_set_json_data['images']

                for per_image_info in per_set_images_info:
                    per_image_name = per_image_info['file_name'].split('/')[-1]

                    self.all_used_image_name_to_image_mask_path_dict[
                        per_image_name] = self.all_root_dir_image_name_to_image_mask_path_dict[
                            per_image_name]

        if read_filtered_json:
            filtered_json_name = 'filter_image_mask_info'
            for per_set_type in set_types:
                filtered_json_name = filtered_json_name + f'_{per_set_type}'
            filtered_json_name = filtered_json_name + '.json'
            filtered_json_name = os.path.join(
                '/root/code/SimpleAICV_pytorch_training_examples_on_ImageNet_COCO_ADE20K/simpleAICV/interactive_segmentation/datasets',
                filtered_json_name)

            with open(filtered_json_name, 'r', encoding='utf-8') as f:
                self.all_image_mask_info_list = json.load(f)
        else:
            self.all_image_mask_info_list = []
            for per_image_name, per_image_mask_paths in tqdm(
                    self.all_used_image_name_to_image_mask_path_dict.items()):
                per_image_path = self.all_used_image_name_to_image_mask_path_dict[
                    per_image_name][0]
                per_image_full_mask_path = per_image_mask_paths[1]
                per_image_object_mask_path = per_image_mask_paths[2]
                per_image_shadow_mask_path = per_image_mask_paths[3]

                per_image_mask = cv2.imdecode(
                    np.fromfile(per_image_full_mask_path, dtype=np.uint8),
                    cv2.IMREAD_COLOR)
                per_image_mask = cv2.cvtColor(per_image_mask,
                                              cv2.COLOR_BGR2RGB)

                # 将图像转换为2D数组，每行是一个像素的RGB颜色
                pixels = per_image_mask.reshape((-1, 3))

                # 找到唯一颜色并计算每种颜色的索引
                unique_colors, indices = np.unique(pixels,
                                                   axis=0,
                                                   return_inverse=True)

                # 创建一个布尔索引数组，用于忽略背景色 [0, 0, 0]
                ignore_color = np.all(unique_colors == [0, 0, 0], axis=1)

                # 删除背景色，并更新唯一颜色和索引
                unique_colors = unique_colors[~ignore_color]

                # 更新indices映射
                for idx, ignore in enumerate(ignore_color):
                    if ignore:
                        indices[indices == idx] = -1

                # 重塑索引数组为图像原始形状
                per_image_mask_indices = indices.reshape(
                    per_image_mask.shape[:2])

                for i, color in enumerate(unique_colors):
                    mask = (per_image_mask_indices == i + 1).astype(np.float32)
                    if np.sum(mask) / float(
                            mask.shape[0] * mask.shape[1]) < area_filter_ratio:
                        continue

                    self.all_image_mask_info_list.append([
                        per_image_name,
                        per_image_path,
                        per_image_full_mask_path,
                        per_image_object_mask_path,
            ...
            if save_filtered_json:
                ...
                with open(filtered_json_name, 'w', encoding='utf-8') as f:
                    json.dump(self.all_image_mask_info_list, f)

        self.load_mask_type = load_mask_type
        self.load_box_type = load_box_type
        ...
        print(f'Dataset Size:{len(self.all_image_mask_info_list)}')

    def __len__(self):
        return len(self.all_image_mask_info_list)

    def __getitem__(self, idx):
        image = self.load_image(idx)
        box_type_mask, mask_type_mask = self.load_box_type_mask_type_mask(idx)

        mask = copy.deepcopy(mask_type_mask)
        ...
        image_mask = np.where(box_type_mask > 0.5, 1.0, 0.0).astype(np.float32)
        area_nums, area_mask, stats, centroids = cv2.connectedComponentsWithStats(
            image_mask.astype(np.uint8), connectivity=8)
        area_mask = np.where(area_mask > 0, 1, 0).astype(np.uint8)
        x, y, w, h = cv2.boundingRect(area_mask)
        image_box = np.array((x, y, x + w, y + h), dtype=np.float32)
        ...
        if image_mask_all_points_num < self.positive_points_num:
            ...
            for _ in range(self.positive_points_num):
                positive_prompt_point.append([
                    np.random.randint(0, size[1]),
                    np.random.randint(0, size[0]), 1
                ])
        ...
        if image_not_mask_all_points_num < self.negative_points_num:
            ...
            for _ in range(self.negative_points_num):
        ...
        if np.sum(prompt_mask) == 0:
            ...
            cv2.fillPoly(prompt_mask, [three_points.astype(np.int32)], 1.0)
    ...
    def load_image(self, idx):
        image = cv2.imdecode(
            np.fromfile(self.all_image_mask_info_list[idx][1], dtype=np.uint8),
            cv2.IMREAD_COLOR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        return image.astype(np.float32)

    def load_box_type_mask_type_mask(self, idx):
        mask_color = self.all_image_mask_info_list[idx][-1]
        mask_color = np.array(mask_color)
        mask_paths = self.all_used_image_name_to_image_mask_path_dict[
            self.all_image_mask_info_list[idx][0]]

        if self.load_box_type == 'full_mask':
            box_type_mask_path = mask_paths[1]
        elif self.load_box_type == 'object_mask':
            box_type_mask_path = mask_paths[2]
        elif self.load_box_type == 'shadow_mask':
            box_type_mask_path = mask_paths[3]

        if self.load_mask_type == 'full_mask':
            mask_type_mask_path = mask_paths[1]
        elif self.load_mask_type == 'object_mask':
            mask_type_mask_path = mask_paths[2]
        elif self.load_mask_type == 'shadow_mask':
            mask_type_mask_path = mask_paths[3]

        box_type_mask = cv2.imdecode(
            np.fromfile(box_type_mask_path, dtype=np.uint8), cv2.IMREAD_COLOR)
        box_type_mask = cv2.cvtColor(box_type_mask, cv2.COLOR_BGR2RGB)
        box_type_mask = np.all(box_type_mask == mask_color, axis=-1)

        mask_type_mask = cv2.imdecode(
            np.fromfile(mask_type_mask_path, dtype=np.uint8), cv2.IMREAD_COLOR)
        mask_type_mask = cv2.cvtColor(mask_type_mask, cv2.COLOR_BGR2RGB)
        mask_type_mask = np.all(mask_type_mask == mask_color, axis=-1)

        return box_type_mask.astype(np.float32), mask_type_mask.astype(
            np.float32)
...
if __name__ == '__main__':
    ...
    soba_v2_dataset = SOBAV2Dataset(
        soba_v2_dataset_path,
        set_types=[
            'train',
            'val',
            'challenge',
    ...
    count = 0
    for per_sample in tqdm(soba_v2_dataset):
    ...
    train_loader = DataLoader(soba_v2_dataset,
                              batch_size=4,
                              shuffle=True,
                              num_workers=2,
                              collate_fn=collater)
