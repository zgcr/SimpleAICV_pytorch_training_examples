...
class SalientObjectDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_mask = np.where(mask > 0.2, 1.0, 0.0).astype(np.float32)
        ...
        if np.sum(image_mask) == 0:
            ...
            y_min, y_max = sorted(
                [np.random.randint(0, size[0]),
                 np.random.randint(0, size[0])])
            image_box = np.array((x_min, y_min, x_max, y_max),
                                 dtype=np.float32)
        else:
...
if __name__ == '__main__':
    ...
    for per_sample in tqdm(salient_object_detection_dataset):
        ...
        # class_name, class_color = FOREGROUND_CLASSES[
        #     0], FOREGROUND_CLASSES_COLOR[0]
