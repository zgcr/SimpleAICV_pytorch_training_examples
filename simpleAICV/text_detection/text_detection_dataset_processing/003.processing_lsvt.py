...
def generate_standard_format_txt_for_each_image(root_dataset_path,
    ...
    root_dataset_image_path = os.path.join(root_dataset_path,
                                           'train_full_images')
    ...
    for per_image_name in os.listdir(root_dataset_image_path):
        ...
        all_image_label_name_path_list.append(
            [per_image_name_prefix, per_image_name, per_image_path])

    print('1111', len(all_image_label_name_path_list),
          all_image_label_name_path_list[0])

    root_dataset_label_path = os.path.join(root_dataset_path,
                                           'train_full_labels.json')
    root_dataset_label = json.load(
        open(root_dataset_label_path, 'r', encoding='UTF-8'))
    print('2222', len(root_dataset_label))

    for key, value in root_dataset_label.items():
        print(key, value)
        break
    ...
    for per_image_name_prefix, per_image_name, per_image_path in tqdm(
            all_image_label_name_path_list):
        if not os.path.exists(per_image_path):
            continue
        ...
        if per_image_name in ['image_6089.jpg']:
            continue

        per_image_origin_labels = root_dataset_label[per_image_name_prefix]

        illegal_box_points_num = False
        per_image_labels = []
        for per_label_line in per_image_origin_labels:
            per_label_box = per_label_line['points']
            per_label_char = per_label_line['transcription']

            if len(per_label_box) < 4:
                illegal_box_points_num = True
                break

            per_label_final_coords = per_label_box
...
if __name__ == '__main__':
    root_dataset_path = r'D:\BaiduNetdiskDownload\text_dataset\ICDAR2019LSVT'
    save_dataset_path = r'D:\BaiduNetdiskDownload\text_dataset_处理后\ICDAR2019LSVT_text_detection'
    train_ratio = 0.8
