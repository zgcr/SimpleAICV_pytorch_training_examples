...
from simpleAICV.human_matting.models import backbones
...
class VANPFANMatting(nn.Module):
    ...
    def __init__(self,
                 ...
                 cpfe_planes=32):
        super(VANPFANMatting, self).__init__()
    ...
    def forward(self, x):
        ...
        conv_34_feats_g = F.upsample(conv_34_feats_g,
        ...
        x4_feat_f = F.upsample(x4_feat_f,
                               size=(x3.shape[2], x3.shape[3]),
                               mode='bilinear',
                               align_corners=True)
        # torch.Size([1, 32, 52, 52])
        ...
        conv_34_feats_f = F.upsample(conv_34_feats_f,
                                     size=(x1.shape[2], x1.shape[3]),
                                     mode='bilinear',
                                     align_corners=True)
        # torch.Size([1, 32, 208, 208])
        ...
        x2_feat_f = F.upsample(x2_feat_f,
                               size=(x1.shape[2], x1.shape[3]),
                               mode='bilinear',
                               align_corners=True)
        # torch.Size([1, 32, 208, 208])
        ...
        # local_pred:[1, 1, 832, 832],3:0为背景区域，1为local区域，2为global区域
...
def _van_pfan_matting(backbone_type, backbone_pretrained_path, planes,
                      **kwargs):
    model = VANPFANMatting(backbone_type=backbone_type,
...
def van_b0_pfan_matting(backbone_pretrained_path='', **kwargs):
    return _van_pfan_matting('van_b0', backbone_pretrained_path,
                             [32, 64, 160, 256], **kwargs)


def van_b1_pfan_matting(backbone_pretrained_path='', **kwargs):
    return _van_pfan_matting('van_b1', backbone_pretrained_path,
                             [64, 128, 320, 512], **kwargs)


def van_b2_pfan_matting(backbone_pretrained_path='', **kwargs):
    return _van_pfan_matting('van_b2', backbone_pretrained_path,
                             [64, 128, 320, 512], **kwargs)
