...
from tools.train_imagenet21k_classification_model import test_classification
from tools.utils import get_logger, set_seed, compute_macs_and_params


def parse_args():
    parser = argparse.ArgumentParser(
        description='PyTorch Imagenet21k Classification Testing')
...
def main():
    ...
    log_info = f'acc1: {acc1:.3f}%, test_loss: {test_loss:.4f}, per_image_load_time: {per_image_load_time:.3f}ms, per_image_inference_time: {per_image_inference_time:.3f}ms'
    logger.info(log_info) if local_rank == 0 else None
