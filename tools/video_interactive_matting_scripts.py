...
def train_sam2_matting(train_loader, model, criterion, optimizer, scheduler,
                       epoch, logger, config):
    losses = AverageMeter()
    ...
    for _, data in enumerate(train_loader):
        ...
        if torch.any(torch.isinf(input_images)) or torch.any(
                torch.isinf(input_masks)):
            skip_batch_flag = True

        if torch.any(torch.isnan(input_images)) or torch.any(
                torch.isnan(input_masks)):
            skip_batch_flag = True

        if config.use_amp:
            with autocast(device_type="cuda", dtype=amp_type):
                outs = model(data)
                loss_value = criterion(input_images, outs, targets)
        ...
        if iter_index % config.accumulation_steps == 0:
            ...
            losses.update(loss, input_images.size(0))
