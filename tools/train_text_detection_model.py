...
from tools.text_scripts import train_text_detection, test_text_detection_for_all_dataset
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='PyTorch Text Detection Training')
    parser.add_argument(
        '--work-dir',
        type=str,
...
def main():
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        train_loss = train_text_detection(train_loader, model, train_criterion,
        ...
        if epoch in config.eval_epoch or epoch == config.epochs:
            result_dict = test_text_detection_for_all_dataset(
                val_loader_list, model, test_criterion, decoder, config)
