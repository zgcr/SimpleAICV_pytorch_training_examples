...
def validate_human_parsing_for_all_dataset(val_loader_list, model, criterion,
                                           config):
...
def validate_human_parsing(test_loader, model, criterion, config):
...
def train_human_parsing(train_loader, model, criterion, optimizer, scheduler,
                        epoch, logger, config):
    '''
    train human parsing model for one epoch
    '''
    losses = AverageMeter()
