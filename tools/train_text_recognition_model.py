...
from tools.text_scripts import train_text_recognition, test_text_recognition_for_all_dataset
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='PyTorch Text Recognition Training')
    parser.add_argument(
        '--work-dir',
        type=str,
...
def main():
    ...
    for key, value in config.__dict__.items():
        if not key.startswith('__'):
            if key not in [
                    ...
                    'num_char_table',
                    'alpha_char_table',
                    'common_standard_chinese_char_first_table',
                    'common_standard_chinese_char_second_table',
                    'common_standard_chinese_char_third_table',
            ]:
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        train_loss = train_text_recognition(train_loader, model,
        ...
        if epoch in config.eval_epoch or epoch == config.epochs:
            result_dict = test_text_recognition_for_all_dataset(
