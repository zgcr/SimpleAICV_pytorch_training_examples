...
from simpleAICV.detection.datasets.vocdataset import VOC_CLASSES, VOC_CLASSES_COLOR

from tools.utils import compute_macs_and_params
...
def parse_args():
    ...
    parser.add_argument('--test_video_path', type=str, help='test video path')
    parser.add_argument("--save_video_path",
                        type=str,
                        help="save detected video path")
    parser.add_argument('--show_video',
                        default=False,
                        action='store_true',
                        help='show_video or not, not avaliable on server')
    args = parser.parse_args()

    return args


def load_image_for_detection_inference(img, args, divisor=32):
...
def inference():
    ...
    macs, params = compute_macs_and_params(args, model)
    print(f'model: {args.model}, macs: {macs}, params: {params}')

    model = model.cuda()
    if 'retinanet' in args.decoder or 'fcos' in args.decoder:
