...
class SalientObjectDetectionSegmentationTestCollater:
...
class SalientObjectDetectionSegmentationTrainCollater:
    ...
    def __call__(self, data):
        ...
        for i in range(len(images)):
            ...
            per_image_input_class_masks = np.zeros(
                (len(per_image_masks), self.resize, self.resize),
                dtype=np.float32)
            per_image_input_class_masks[:, 0:per_image_masks.shape[1],
                                        0:per_image_masks.
                                        shape[2]] = per_image_masks
            per_image_input_class_masks = torch.from_numpy(
                per_image_input_class_masks)
            per_image_input_class_masks = per_image_input_class_masks.float()

            # 每张图像只有一个主体mask,类别标签设为0
            per_image_labels = np.array([0]).astype(np.float32)
            per_image_labels = torch.from_numpy(per_image_labels).long()

            input_masks.append(per_image_input_class_masks)
