...
class VITMAEPretrainModelEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, (self.image_size // self.patch_size)**2 + 1,
                        self.embedding_planes),
            requires_grad=False)
...
if __name__ == '__main__':
    ...
    print(
        f'1111, flops: {flops}, macs: {macs}, params: {params}, out_shape: {out.shape}, mask_shape: {mask.shape}'
    )
    ...
    net = vit_huge_patch14_224_mae_pretrain_model(use_gradient_checkpoint=True)
    image_h, image_w = 224, 224
    out, mask = net(
        torch.autograd.Variable(torch.randn(3, 3, image_h, image_w)))
    print(f'2222, out_shape: {out.shape}, mask_shape: {mask.shape}')
