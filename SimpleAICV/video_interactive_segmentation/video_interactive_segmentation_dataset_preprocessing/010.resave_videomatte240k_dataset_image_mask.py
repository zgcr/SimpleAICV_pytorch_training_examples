...
def process_single_video(args):
    """处理单个视频的函数"""
    per_video_name, per_video_image_dir_path, per_video_mask_dir_path, per_video_image_path_list, per_video_mask_path_list, save_path, root_name, set_type = args
    ...
    first_frame_image = cv2.imdecode(
        np.fromfile(first_frame_image_path, dtype=np.uint8), cv2.IMREAD_COLOR)
    ...
    for per_frame_image_path in per_video_image_path_list:
        ...
        if need_resize:
            per_frame_image = cv2.resize(per_frame_image, (resize_w, resize_h))

        per_video_all_images.append(per_frame_image)

    per_video_all_masks = []
    for per_frame_mask_path in per_video_mask_path_list:
        per_frame_mask = np.array(Image.open(per_frame_mask_path).convert('L'),
                                  dtype=np.uint8)

        if need_resize:
            per_frame_mask = cv2.resize(per_frame_mask, (resize_w, resize_h),
                                        interpolation=cv2.INTER_NEAREST)

        # 0.9*255
        per_frame_mask[per_frame_mask >= 230] = 255
        # 0.1*255
        per_frame_mask[per_frame_mask <= 25] = 0
        per_frame_mask = per_frame_mask / 255.
        per_video_all_masks.append(per_frame_mask)
    ...
    for idx in range(frame_nums):
        ...
        per_frame_invalid_flag = get_frame_invalid_flag(per_frame_mask,
                                                        min_ratio=0.0001,
                                                        max_ratio=0.9)
    ...
    # 更新images和masks
    valid_per_video_all_images = per_video_all_images[start_idx:end_idx + 1]
    valid_per_video_all_masks = per_video_all_masks[start_idx:end_idx + 1]

    if len(valid_per_video_all_images) < len(per_video_all_images):
        print(
            f'video_name:{per_video_name}, frame_num:{len(per_video_all_images)}, filtered_frame_num: {len(valid_per_video_all_images)}'
        )
    ...
    for idx, per_mask in enumerate(valid_per_video_all_masks):
        ...
        new_mask_name = f'{root_name}_{set_type}_{per_video_name}_{idx:05d}.png'
...
def process_videos(root_path, save_path, set_type):
    ...
    root_dataset_path = os.path.join(root_path, set_type, 'fgr')
    ...
    for per_video_name in sorted(os.listdir(root_dataset_path)):
        ...
        mask_dataset_path = os.path.join(root_path, set_type, 'pha')
    ...
    print('1111', len(all_video_path_list))
    ...
    #             Image.open(per_frame_mask_path).convert('L'), dtype=np.uint8)

    #         if need_resize:
    #             per_frame_mask = cv2.resize(per_frame_mask,
    #                                         (resize_w, resize_h),
    #                                         interpolation=cv2.INTER_NEAREST)
    ...
    #     # 更新images和masks
    #     valid_per_video_all_images = per_video_all_images[start_idx:end_idx +
    #                                                       1]
    #     valid_per_video_all_masks = per_video_all_masks[start_idx:end_idx + 1]
...
if __name__ == "__main__":
    root_path = r'/root/autodl-tmp/VideoMatte240K'
    save_path = r'/root/autodl-tmp/video_interactive_segmentation_dataset/VideoMatte240K'
    set_type = 'test'
