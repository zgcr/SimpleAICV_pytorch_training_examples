...
__all__ = [
    'dinov3_vit_small_patch16_sam2video_matting_test',
    'dinov3_vit_small_plus_patch16_sam2video_matting_test',
    'dinov3_vit_base_patch16_sam2video_matting_test',
    'dinov3_vit_large_patch16_sam2video_matting_test',
    'dinov3_vit_large_plus_patch16_sam2video_matting_test',
    'dinov3_vit_huge_plus_patch16_sam2video_matting_test',
]


class DINOV3SAM2VideoMattingTest(nn.Module):
    ...
    def __init__(self,
        ...
        super(DINOV3SAM2VideoMattingTest, self).__init__()
        ...
        with torch.inference_mode():
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16):
                ...
                if start_tracking_frame_idx == 0 or start_tracking_frame_idx == video_frame_num - 1:
                    ...
                    all_frame_global_preds, all_frame_local_preds, all_frame_fused_preds, all_frame_iou_preds, all_frame_pred_object_score_logits = self.process_one_video_frame_sequence(
                        video_state_dict,
                    ...
                    for per_frame_idx, per_frame_global_pred, per_frame_local_pred, per_frame_fused_pred, per_frame_iou_preds, per_frame_pred_object_score_logits in zip(
                        ...
                        per_frame_global_pred_origin_size = []
                        per_frame_local_pred_origin_size = []
                        per_frame_fused_pred_origin_size = []
                        for idx in range(per_frame_global_pred.shape[1]):
                            global_preds_per_out_idx = per_frame_global_pred[:,
                                                                             idx, :, :, :]
                            local_preds_per_out_idx = per_frame_local_pred[:,
                                                                           idx, :, :, :]
                            fused_preds_per_out_idx = per_frame_fused_pred[:,
                                                                           idx, :, :, :]
                            # 恢复原始尺寸
                            global_preds_per_out_idx = F.interpolate(
                                global_preds_per_out_idx,
                                (video_height, video_width),
                                mode="bilinear")
                            local_preds_per_out_idx = F.interpolate(
                                local_preds_per_out_idx,
                                (video_height, video_width),
                                mode="bilinear")
                            fused_preds_per_out_idx = F.interpolate(
                                fused_preds_per_out_idx,
                                (video_height, video_width),
                                mode="bilinear")

                            per_frame_global_pred_origin_size.append(
                                global_preds_per_out_idx)
                            per_frame_local_pred_origin_size.append(
                                local_preds_per_out_idx)
                            per_frame_fused_pred_origin_size.append(
                                fused_preds_per_out_idx)

                        per_frame_global_pred_origin_size = torch.stack(
                            per_frame_global_pred_origin_size, dim=1)
                        per_frame_local_pred_origin_size = torch.stack(
                            per_frame_local_pred_origin_size, dim=1)
                        per_frame_fused_pred_origin_size = torch.stack(
                            per_frame_fused_pred_origin_size, dim=1)

                        for per_object_id in tracking_object_ids:
                            per_object_id_idx = tracking_object_ids.index(
                                per_object_id)
                            per_frame_per_object_global_pred = per_frame_global_pred_origin_size[
                                per_object_id_idx][0].cpu().float().numpy()
                            per_frame_per_object_local_pred = per_frame_local_pred_origin_size[
                                per_object_id_idx][0].cpu().float().numpy()
                            per_frame_per_object_fused_pred = per_frame_fused_pred_origin_size[
                                per_object_id_idx][0].cpu().float().numpy()
                            ...
                            video_state_dict['object_track_result'][
                                per_object_id][per_frame_idx] = {
                                    'global_pred':
                                    per_frame_per_object_global_pred,
                                    'local_pred':
                                    per_frame_per_object_local_pred,
                                    'fuse_pred':
                                    per_frame_per_object_fused_pred,
    ...
    def process_one_video_frame_sequence(self,
        ...
        for process_frame_idx in processing_frame_idxs:
            ...
            process_frame_preds = {}
            process_frame_preds['global_preds'] = process_frame_global_preds
            process_frame_preds['local_preds'] = process_frame_local_preds
            process_frame_preds['fused_preds'] = process_frame_fused_preds
            process_frame_preds['iou_preds'] = process_frame_iou_preds
            process_frame_preds[
        ...
        with torch.inference_mode():
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16):
                ...
                for idx, mask_out_idx in enumerate(mask_out_idxs):
                    ...
                    # 恢复原始尺寸
                    global_preds_per_out_idx = F.interpolate(
                        global_preds_per_out_idx, (video_height, video_width),
                        mode="bilinear")
                    local_preds_per_out_idx = F.interpolate(
                        local_preds_per_out_idx, (video_height, video_width),
                        mode="bilinear")
                    fused_preds_per_out_idx = F.interpolate(
                        fused_preds_per_out_idx, (video_height, video_width),
                        mode="bilinear")
...
def _dinov3_sam2videomattingtest(backbone_type, backbone_pretrained_path,
                                 image_size, patch_size,
                                 prompt_encoder_embedding_planes, **kwargs):
    model = DINOV3SAM2VideoMattingTest(
        backbone_type=backbone_type,
        backbone_pretrained_path=backbone_pretrained_path,
        image_size=image_size,
        patch_size=patch_size,
...
def dinov3_vit_small_patch16_sam2video_matting_test(backbone_pretrained_path='',
                                                    image_size=1024,
                                                    patch_size=16,
                                                    **kwargs):
    return _dinov3_sam2videomattingtest(
...
def dinov3_vit_small_plus_patch16_sam2video_matting_test(
        backbone_pretrained_path='', image_size=1024, patch_size=16, **kwargs):
    return _dinov3_sam2videomattingtest(
...
def dinov3_vit_base_patch16_sam2video_matting_test(backbone_pretrained_path='',
                                                   image_size=1024,
                                                   patch_size=16,
                                                   **kwargs):
    return _dinov3_sam2videomattingtest(
...
def dinov3_vit_large_patch16_sam2video_matting_test(backbone_pretrained_path='',
                                                    image_size=1024,
                                                    patch_size=16,
                                                    **kwargs):
    return _dinov3_sam2videomattingtest(
...
def dinov3_vit_large_plus_patch16_sam2video_matting_test(
        backbone_pretrained_path='', image_size=1024, patch_size=16, **kwargs):
    return _dinov3_sam2videomattingtest(
...
def dinov3_vit_huge_plus_patch16_sam2video_matting_test(
        backbone_pretrained_path='', image_size=1024, patch_size=16, **kwargs):
    return _dinov3_sam2videomattingtest(
