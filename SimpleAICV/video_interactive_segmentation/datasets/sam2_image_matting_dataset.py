...
class SAM2ImageMattingDataset(Dataset):
...
if __name__ == '__main__':
    ...
    from SimpleAICV.video_interactive_segmentation.common_matting import Sam2MattingResize, Sam2MattingRandomHorizontalFlip, Sam2MattingRandomMosaicAug, Sam2MattingRandomRsverseFrameOrder, Sam2MattingNormalize, SAM2MattingBatchCollater

    sam2_video_dataset = SAM2ImageMattingDataset(
        ...
        transform=transforms.Compose([
            ...
            Sam2MattingRandomMosaicAug(prob=0.5),
            Sam2MattingRandomRsverseFrameOrder(prob=0.5),
    ...
    for per_sample in tqdm(sam2_video_dataset):
        ...
        print('2222', np.max(per_sample['mask']), np.min(per_sample['mask']))
        print('3333', np.max(per_sample['trimap']),
              np.min(per_sample['trimap']), np.unique(per_sample['trimap']))
        print('4444', np.max(per_sample['fg_map']),
              np.min(per_sample['fg_map']))
        print('5555', np.max(per_sample['bg_map']),
        ...
        # per_sample_trimaps = per_sample['trimap']
        # per_sample_fg_maps = per_sample['fg_map']
        # per_sample_bg_maps = per_sample['bg_map']
        ...
        #     per_object_trimaps = per_sample_trimaps[:, :, :, per_object_idx]
        #     per_object_fg_maps = per_sample_fg_maps[:, :, :, :, per_object_idx]
        #     per_object_bg_maps = per_sample_bg_maps[:, :, :, :, per_object_idx]
    ...
    collater = SAM2MattingBatchCollater(resize=1024)
    ...
    for data in tqdm(train_loader):
        ...
        print('2222', batch_images.shape, input_masks.shape,
        ...
        print('2222', batch_images.dtype, input_masks.dtype,
