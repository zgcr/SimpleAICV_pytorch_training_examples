...
class SAMMattingRandomScale:
    ...
    def __call__(self, sample):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            new_prompt_point = []
            for per_prompt_point in prompt_point:
                per_prompt_point_label = per_prompt_point[2]
                per_prompt_point_coord = per_prompt_point[0:2]
                per_prompt_point_coord = self.transform_point(
                    per_prompt_point_coord, M)

                new_per_prompt_point = [
                    per_prompt_point_coord[0],
                    per_prompt_point_coord[1],
                    per_prompt_point_label,
                ]
                new_per_prompt_point = np.array(new_per_prompt_point,
                                                dtype=np.float32)
                new_prompt_point.append(new_per_prompt_point)

            new_prompt_point = np.array(new_prompt_point, dtype=np.float32)
            prompt_point = new_prompt_point
...
class SAMMattingRandomVerticalFlip:
    ...
    def __call__(self, sample):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            for i in range(len(prompt_point)):
                prompt_point[i][1] = h - prompt_point[i][1]
