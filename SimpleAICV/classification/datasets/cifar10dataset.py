...
if __name__ == '__main__':
    ...
    count = 0
    for per_sample in tqdm(cifar10traindataset):
    ...
    train_loader = DataLoader(cifar10traindataset,
                              batch_size=128,
                              shuffle=True,
                              num_workers=4,
                              collate_fn=collater)
    ...
    cifar10testdataset = CIFAR10Dataset(root_dir=CIFAR10_path,
                                        set_name='test',
                                        transform=transforms.Compose([
                                            Opencv2PIL(),
    ...
    count = 0
    for per_sample in tqdm(cifar10testdataset):
        print(per_sample['image'].shape, per_sample['label'].shape,
              per_sample['label'], type(per_sample['image']),
              type(per_sample['label']))
        if count < 2:
    ...
    from torch.utils.data import DataLoader
    collater = ClassificationCollater()
    val_loader = DataLoader(cifar10testdataset,
                            batch_size=128,
                            shuffle=False,
                            num_workers=4,
                            collate_fn=collater)
