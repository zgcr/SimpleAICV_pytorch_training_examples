...
class YOLACTLoss(nn.Module):
    ...
    def forward(self, preds, gt_bboxes, gt_masks):
        ...
        gt_bboxes = [
            per_image_gt_bboxes.float().to(device)
            for per_image_gt_bboxes in gt_bboxes
        ]
        gt_masks = [
            per_image_gt_masks.float().to(device)
            for per_image_gt_masks in gt_masks
        ]
        ...
        batch_anchors = batch_anchors.float()
    ...
    def compute_batch_cls_loss(self,
        ...
        mark = torch.logsumexp(batch_conf, dim=1) - batch_conf[:, 0]
        ...
        cls_loss = F.cross_entropy(filter_class_preds,
                                   filter_class_gts,
                                   reduction='sum')
        positive_num = positive_sample_num.sum()
        cls_loss = cls_loss / positive_num
...
class SOLOV2Loss(nn.Module):
    ...
    def compute_batch_dice_loss(self, mask_feat_pred, kernel_preds,
                                batch_instance_label_list,
                                batch_grid_order_list):
        mask_feat_pred = mask_feat_pred.float()
