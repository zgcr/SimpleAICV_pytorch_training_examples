...
__all__ = [
    'resnet18_pfan_face_parsing',
    'resnet34_pfan_face_parsing',
    'resnet50_pfan_face_parsing',
    'resnet101_pfan_face_parsing',
    'resnet152_pfan_face_parsing',
    'vanb0_pfan_face_parsing',
    'vanb1_pfan_face_parsing',
    'vanb2_pfan_face_parsing',
    'vanb3_pfan_face_parsing',
    'convformers18_pfan_face_parsing',
    'convformers36_pfan_face_parsing',
    'convformerm36_pfan_face_parsing',
    'convformerb36_pfan_face_parsing',
]
...
def _pfan_face_parsing(backbone_type, backbone_pretrained_path, planes,
                       **kwargs):
...
def resnet18_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('resnet18backbone', backbone_pretrained_path,
                              [64, 128, 256, 512], **kwargs)


def resnet34_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('resnet34backbone', backbone_pretrained_path,
                              [64, 128, 256, 512], **kwargs)


def resnet50_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('resnet50backbone', backbone_pretrained_path,
                              [256, 512, 1024, 2048], **kwargs)


def resnet101_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('resnet101backbone', backbone_pretrained_path,
                              [256, 512, 1024, 2048], **kwargs)


def resnet152_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('resnet152backbone', backbone_pretrained_path,
                              [256, 512, 1024, 2048], **kwargs)


def vanb0_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('vanb0backbone', backbone_pretrained_path,
                              [32, 64, 160, 256], **kwargs)


def vanb1_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('vanb1backbone', backbone_pretrained_path,
                              [64, 128, 320, 512], **kwargs)


def vanb2_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('vanb2backbone', backbone_pretrained_path,
                              [64, 128, 320, 512], **kwargs)


def vanb3_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('vanb3backbone', backbone_pretrained_path,
                              [64, 128, 320, 512], **kwargs)


def convformers18_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('convformers18backbone',
                              backbone_pretrained_path, [64, 128, 320, 512],
                              **kwargs)


def convformers36_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('convformers36backbone',
                              backbone_pretrained_path, [64, 128, 320, 512],
                              **kwargs)


def convformerm36_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('convformerm36backbone',
                              backbone_pretrained_path, [96, 192, 384, 576],
                              **kwargs)


def convformerb36_pfan_face_parsing(backbone_pretrained_path='', **kwargs):
    return _pfan_face_parsing('convformerb36backbone',
...
if __name__ == '__main__':
    ...
    net = resnet18_pfan_face_parsing()
    ...
    net = resnet34_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
    ...
    net = resnet50_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
    ...
    net = resnet101_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
    ...
    net = resnet152_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
    ...
    net = vanb3_pfan_face_parsing(use_gradient_checkpoint=True)
    image_h, image_w = 512, 512
    outs = net(torch.autograd.Variable(torch.randn(3, 3, image_h, image_w)))
    out = net(torch.rand(1, 3, image_h, image_w))
    print('2222', out.shape, torch.max(out), torch.min(out))

    net = convformers18_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
    ...
    net = convformers36_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
    ...
    net = convformerm36_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
    ...
    net = convformerb36_pfan_face_parsing()
    image_h, image_w = 512, 512
    from calflops import calculate_flops
    flops, macs, params = calculate_flops(model=net,
                                          kwargs={
