...
class DBNetTextDetectionCollater:
    ...
    def __call__(self, data):
        ...
        probability_mask = torch.from_numpy(probability_mask)
        probability_mask = probability_mask.float()

        probability_ignore_mask = torch.from_numpy(probability_ignore_mask)
        probability_ignore_mask = probability_ignore_mask.float()

        threshold_mask = torch.from_numpy(threshold_mask)
        threshold_mask = threshold_mask.float()

        threshold_ignore_mask = torch.from_numpy(threshold_ignore_mask)
        threshold_ignore_mask = threshold_ignore_mask.float()
