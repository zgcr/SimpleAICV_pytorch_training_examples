...
def train_network(config, net_info):
    assert torch.cuda.is_available(), 'need gpu to train network!'
    torch.cuda.empty_cache()

    net_idx = net_info['net_num_idx']
    net_config = net_info['net_config']
    log_dir = os.path.join(config.log_dir, f'{net_idx}')
    checkpoint_dir = os.path.join(config.checkpoint_dir, f'{net_idx}')
    resume_model = os.path.join(checkpoint_dir, 'latest.pth')
    print("1313", net_idx, net_config, log_dir, checkpoint_dir, resume_model)
    ...
    logger = get_logger(f'train_{net_idx}', log_dir)
    ...
    log_info = f'net_idx: {net_idx}'
    logger.info(log_info) if local_rank == 0 else None
    log_info = f'net_config: {net_config}'
    logger.info(log_info) if local_rank == 0 else None
    ...
    for key, value in config.__dict__.items():
        if not key.startswith('__'):
            if key not in [
                    'model',
                    'sample_net_config',
                    'candidate_nets_config',
            ]:
                log_info = f'{key}: {value}'
                logger.info(log_info) if local_rank == 0 else None

    model = RegNet(**{
        'regnet_config': net_config,
    ...
    if local_rank == 0:
        if os.path.exists(os.path.join(checkpoint_dir, 'best.pth')):
            os.rename(os.path.join(checkpoint_dir, 'best.pth'),
                      os.path.join(checkpoint_dir, f'acc{best_acc1:.3f}.pth'))

    log_info = f'train done. train time: {train_time:.3f} hours, best_acc1: {best_acc1:.3f}%'
    logger.info(log_info) if local_rank == 0 else None

    return test_loss, best_acc1
...
def main():
    ...
    config.log_dir = os.path.join(args.work_dir, 'log')
    config.checkpoint_dir = os.path.join(args.work_dir, 'checkpoints')
    ...
    if local_rank == 0:
        os.makedirs(config.checkpoint_dir) if not os.path.exists(
            config.checkpoint_dir) else None
        os.makedirs(
            config.log_dir) if not os.path.exists(config.log_dir) else None

    torch.distributed.barrier()

    logger = get_logger('records', config.log_dir)

    for _, net_info in config.candidate_nets_config.items():
        print(f"train_net: {net_info}")
        test_loss, best_acc1 = train_network(config, net_info)
        net_idx, net_config = net_info['net_num_idx'], net_info['net_config']
        if local_rank == 0:
            log_info = f'net_idx: {net_idx}, test_loss: {test_loss:.4f}, best_acc1: {best_acc1:.3f}%, net_config: {net_config}'
            logger.info(log_info) if local_rank == 0 else None
